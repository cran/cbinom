...
F(x) = \code{1 - pbeta(prob, x, size - x + 1)}.

If an element of \code{x} is not in \code{[0, N + 1]}, the result of
\code{dcbinom()} is zero, with a warning. The PDF \code{dcbinom(x, size, prob)}
is computed via numerical differentiation of the CDF =
\code{1 - pbeta(prob, x, size - x + 1)}.}
...
# Use "log = TRUE" for an extended range:
n <- 1000
k <- seq(0, n, by = 20)
cbind(dcbinom(k, n, .481, log = TRUE), log(dcbinom(k, n, .481)))
}